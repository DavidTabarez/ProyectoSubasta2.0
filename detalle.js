...
function renderizarProducto(p) {
  ...
  let extrasHTML = "";
  if (p.marca)                extrasHTML += fila("Marca / Modelo", `${p.marca} ${p.modelo || ""} ${p.anio || ""}`);
  if (p.kilometraje)          extrasHTML += fila("Kilometraje", `${p.kilometraje.toLocaleString("es-MX")} km`);
  if (p.numeroSerie)          extrasHTML += fila("No. de serie", p.numeroSerie);
  if (p.condicionMecanica)    extrasHTML += fila("Condición mecánica", p.condicionMecanica);
  if (p.tipoPropiedad)        extrasHTML += fila("Tipo de propiedad", p.tipoPropiedad);
  if (p.superficieConstruida) extrasHTML += fila("Superficie construida", `${p.superficieConstruida} m²`);
  if (p.superficieTerreno)    extrasHTML += fila("Superficie terreno", `${p.superficieTerreno} m²`);
  if (p.habitaciones)         extrasHTML += fila("Recámaras", p.habitaciones);
  if (p.documentacionLegal !== undefined) extrasHTML += fila("Documentación legal", p.documentacionLegal ? "✅ Completa" : "⚠️ Pendiente");
